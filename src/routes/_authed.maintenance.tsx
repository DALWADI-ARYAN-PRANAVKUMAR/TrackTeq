...
export const Route = createFileRoute("/_authed/maintenance")({
  head: () => ({ meta: [{ title: "Maintenance — Track-Teq" }] }),
  component: MaintPage,
});
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild><Button size="sm"><Plus className="h-3.5 w-3.5 mr-1" /> Open log</Button></DialogTrigger>
          <DialogContent>
                ...
                <Select value={form.vehicleId} onValueChange={(v) => setForm({ ...form, vehicleId: v })}>
                  ...
                  <SelectContent>
                    {vehicles.filter((v) => v.status !== "On Trip" && v.status !== "Retired").map((v) => (
                      <SelectItem key={v.id} value={v.id}>{v.reg} — {v.name}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
              <div><Label className="micro-label">Type</Label><Input value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value })} placeholder="Oil change, brakes..." /></div>
              <div><Label className="micro-label">Notes</Label><Input value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} /></div>
              <div><Label className="micro-label">Cost (₹)</Label><Input type="number" value={form.cost} onChange={(e) => setForm({ ...form, cost: +e.target.value })} /></div>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="rounded-sm border border-border bg-panel">
          ...
          <div className="divide-y divide-border">
                ...
                <div key={m.id} className="p-4">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <div className="text-[11px] font-mono text-muted-foreground">{v?.reg} · {v?.name}</div>
                    </div>
                  ...
                  <div className="mt-3 flex items-center justify-between">
                    ...
                    <Button size="sm" variant="outline" onClick={async () => { await closeMaintenance(m.id); toast.success("Closed — vehicle restored"); }}>
                      Close log
                    </Button>
                  </div>
                </div>
            ...
            {openLogs.length === 0 && <div className="p-8 text-center text-sm text-muted-foreground">No open logs.</div>}
          </div>
        </div>
        ...
        <div className="rounded-sm border border-border bg-panel">
          ...
          <div className="divide-y divide-border max-h-[600px] overflow-y-auto">
            ...
            {closedLogs.length === 0 && <div className="p-8 text-center text-sm text-muted-foreground">No history yet.</div>}
          </div>
        </div>
      </div>
    </div>
