...
export const Route = createFileRoute("/_authed/reports")({
  head: () => ({ meta: [{ title: "Reports — Track-Teq" }] }),
  component: Reports,
});
...
function Reports() {
  ...
  const perVehicle = useMemo(() => vehicles.map((v) => {
    ...
    const maintC = maintenance.filter((m) => m.vehicleId === v.id).reduce((s, m) => s + m.cost, 0);
    const efficiency = fuelL > 0 ? +(km / fuelL).toFixed(2) : 0;
    const opCost = fuelC + maintC;
    const roi = v.acquisitionCost > 0 ? +(((revenue - opCost) / v.acquisitionCost) * 100).toFixed(2) : 0;
    return {
      reg: v.reg, model: v.name, type: v.type, state: v.region ?? "", city: "",
      km, revenue, fuelLiters: fuelL, opCost, efficiencyKmPerL: efficiency, roiPercent: roi,
    };
  }), [vehicles, trips, fuel, maintenance]);

  const dispatchSummary = useMemo(() => trips.map((t) => {
  ...
  const dispatchTemplate = [{
    tripCode: "TRP-XXXX", status: "Draft",
    sourceCity: "Mumbai", sourceState: "Maharashtra",
    destinationCity: "Pune", destinationState: "Maharashtra",
    plannedKm: 0, actualKm: "", cargoKg: 0, fuelLiters: "", revenueINR: "",
    vehicleReg: "MH 12 AB 0000", vehicleModel: "Tata Ace Gold", vehicleState: "Maharashtra",
    driverName: "", driverLicense: "", driverState: "",
    createdAt: new Date().toISOString(), dispatchedAt: "", completedAt: "",
  }];

  const fleetUtil = Math.round((vehicles.filter((v) => v.status === "On Trip").length / (vehicles.length || 1)) * 100);
  const avgEff = +(perVehicle.filter((x) => x.efficiencyKmPerL > 0).reduce((s, x) => s + x.efficiencyKmPerL, 0) / (perVehicle.filter((x) => x.efficiencyKmPerL > 0).length || 1)).toFixed(2);
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" size="sm" onClick={() => exportCSV("fleet-report.csv", perVehicle)}>
            <FileText className="h-3.5 w-3.5 mr-1" /> Fleet CSV
          </Button>
          <Button variant="outline" size="sm" onClick={() => exportCSV("dispatch-summary.csv", dispatchSummary)}>
            <MapPin className="h-3.5 w-3.5 mr-1" /> Dispatch summary
          </Button>
          <Button variant="secondary" size="sm" onClick={() => exportCSV("dispatch-template.csv", dispatchTemplate)}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <KpiCard label="Total Revenue" value={`₹${totalRev.toLocaleString()}`} icon={TrendingUp} tone="primary" />
        <KpiCard label="Op Cost" value={`₹${totalCost.toLocaleString()}`} icon={DollarSign} tone="accent" />
      </div>
          ...
          <div className="h-72 mt-2">
            <ResponsiveContainer>
              <BarChart data={perVehicle.filter((x) => x.efficiencyKmPerL > 0)}>
                ...
                <XAxis dataKey="reg" stroke="var(--color-muted-foreground)" fontSize={9} tickLine={false} axisLine={false} />
                <YAxis stroke="var(--color-muted-foreground)" fontSize={10} tickLine={false} axisLine={false} />
                <Tooltip contentStyle={{ background: "var(--color-panel)", border: "1px solid var(--color-border)", fontSize: 11 }} />
          ...
          <div className="h-64 mt-2">
            <ResponsiveContainer>
              <LineChart data={perVehicle}>
                ...
                <YAxis stroke="var(--color-muted-foreground)" fontSize={10} tickLine={false} axisLine={false} />
                <Tooltip contentStyle={{ background: "var(--color-panel)", border: "1px solid var(--color-border)", fontSize: 11 }} />
                <Line type="monotone" dataKey="roiPercent" stroke="var(--color-primary)" strokeWidth={2} dot={{ fill: "var(--color-primary)", r: 3 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="rounded-md border border-border bg-panel overflow-hidden">
        <div className="border-b border-border px-4 py-3 micro-label">Per-Vehicle Report (with state)</div>
        <table className="w-full text-sm">
          <thead className="bg-secondary/30">
            <tr>{["Reg", "Model", "State", "Km", "Revenue", "Op Cost", "km/L", "ROI %"].map((h) => (
          ...
          <tbody className="divide-y divide-border">
            {perVehicle.map((v) => (
              <tr key={v.reg}>
                ...
                <td className={`px-4 py-2 font-mono text-xs ${v.roiPercent > 0 ? "text-primary" : v.roiPercent < 0 ? "text-destructive" : ""}`}>
