...
export function KpiCard({
  label, value, unit, icon: Icon, trend, tone = "default",
}: {
  label: string;
  value: string | number;
  unit?: string;
  ...
  const toneCls =
    ...
    tone === "info" ? "text-[oklch(0.72_0.15_220)]" : "text-foreground";
  return (
    <div className="group relative overflow-hidden rounded-md border border-border bg-panel p-4 hover-lift animate-slide-up">
      ...
      <div className="mt-3 flex items-baseline gap-1">
        <span className={cn("text-mono text-3xl font-semibold tabular-nums", toneCls)}>{value}</span>
