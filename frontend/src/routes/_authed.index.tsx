...
import { Truck, CheckCircle2, Wrench, Route as RouteIcon, Clock, Users, Gauge, AlertTriangle } from "lucide-react";
import {
...
function Dashboard() {
  ...
  const alerts = [
    ...drivers
      .map((d) => ({ d, days: Math.floor((new Date(d.licenseExpiry).getTime() - Date.now()) / 86400_000) }))
      .filter((x) => x.days < 45)
      .map((x) => ({
    ...
    ...vehicles
      ...
      .map((v) => ({ kind: "info" as const, title: "In maintenance", detail: `${v.reg} · ${v.name}`, meta: "IN SHOP" })),
  ].slice(0, 6);
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap gap-2">
          <Select value={typeFilter} onValueChange={setTypeFilter}>
            ...
            <SelectContent>
              ...
              {["Van", "Truck", "Semi", "Pickup", "Refrigerated"].map((t) => <SelectItem key={t} value={t}>{t}</SelectItem>)}
            </SelectContent>
          </Select>
          <Select value={statusFilter} onValueChange={setStatusFilter}>
            ...
            <SelectContent>
              ...
              {["Available", "On Trip", "In Shop", "Retired"].map((t) => <SelectItem key={t} value={t}>{t}</SelectItem>)}
            </SelectContent>
          </Select>
          <Select value={regionFilter} onValueChange={setRegionFilter}>
            ...
            <SelectContent>
              ...
              {INDIAN_STATES.map((t) => <SelectItem key={t} value={t}>{t}</SelectItem>)}
            </SelectContent>
          </Select>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="rounded-sm border border-border bg-panel p-4">
          ...
          <div className="mt-2 h-56">
            <ResponsiveContainer>
              <PieChart>
                <Pie data={statusData} dataKey="value" innerRadius={55} outerRadius={80} stroke="none">
                  {statusData.map((d, i) => <Cell key={i} fill={d.color} />)}
                </Pie>
                <Tooltip contentStyle={{ background: "oklch(0.20 0.010 130)", border: "1px solid oklch(0.30 0.010 130)", borderRadius: 2, fontSize: 11 }} />
              </PieChart>
            </ResponsiveContainer>
          </div>
          <div className="grid grid-cols-2 gap-2 mt-2 text-[11px] font-mono">
            {statusData.map((d) => (
              <div key={d.name} className="flex items-center gap-2">
                ...
                <span className="text-muted-foreground uppercase tracking-wider text-[10px]">{d.name}</span>
          ...
          <div className="mt-2 h-56">
            <ResponsiveContainer>
              <BarChart data={tripsPerDay}>
                ...
                <Tooltip contentStyle={{ background: "var(--panel)", border: "1px solid var(--border)", borderRadius: 2, fontSize: 11 }} cursor={{ fill: "var(--chart-1) / 0.1" }} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-1"><RouteMap /></div>
        <div className="lg:col-span-2 rounded-md border border-border bg-panel">
          ...
          <div className="divide-y divide-border">
            {activeFeed.length === 0 && (
              <div className="p-8 text-center text-sm text-muted-foreground">No trips currently dispatched.</div>
            )}
            {activeFeed.map((t) => {
              ...
              return (
                <div key={t.id} className="grid grid-cols-12 items-center gap-3 px-4 py-3 text-sm">
                  ...
                  <div className="col-span-5">
                    <div className="font-medium">{t.source} <span className="text-muted-foreground">→</span> {t.destination}</div>
                  ...
                  <div className="col-span-1"><StatusPill status={t.status} /></div>
                </div>
              );
            })}
          </div>
      ...
      <div className="rounded-md border border-border bg-panel">
        ...
        <div className="divide-y divide-border">
          {alerts.length === 0 && <div className="p-6 text-center text-sm text-muted-foreground">All clear.</div>}
          {alerts.map((a, i) => (
            <div key={i} className="flex items-start gap-3 px-4 py-3">
              <span className={`mt-1 h-2 w-2 rounded-full ${
                a.kind === "critical" ? "bg-destructive" : a.kind === "warn" ? "bg-warn" : "bg-info"
              }`} />
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium">{a.title}</div>
                <div className="text-[11px] text-muted-foreground truncate font-mono">{a.detail}</div>
              </div>
              <span className="text-[10px] font-mono text-muted-foreground uppercase">{a.meta}</span>
            </div>
          ))}
        </div>
      </div>
