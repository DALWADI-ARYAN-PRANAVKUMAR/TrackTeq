...
export const Route = createFileRoute("/_authed/settings")({
  head: () => ({ meta: [{ title: "Settings — Track-Teq" }] }),
  component: Settings,
});

function Settings() {
  const { session, login, theme, setTheme, reducedMotion, setReducedMotion, resetDemo } = useStore();
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <div className="rounded-md border border-border bg-panel p-5">
        ...
        <div className="mt-2 text-sm">
          ...
          <div><span className="text-muted-foreground">Email:</span> <span className="font-mono">{session?.email}</span></div>
        </div>
        <div className="mt-4">
          ...
          <div className="flex flex-wrap gap-2">
            {roles.map((r) => (
              <Button
                ...
                size="sm"
                onClick={() => { if (session) { login({ ...session, role: r }); toast.success(`Now signed in as ${r}`); } }}
              >
                {r}
              </Button>
            ))}
      ...
      <div className="rounded-md border border-border bg-panel p-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <Label htmlFor="rm" className="mt-2 block text-sm font-medium">Reduce motion</Label>
            <p className="mt-1 text-xs text-muted-foreground">
              Disables the cursor companion, aurora shimmer and heavy transitions across the app.
            </p>
          </div>
          <Switch id="rm" checked={reducedMotion} onCheckedChange={(v) => {
            setReducedMotion(v);
      ...
      <div className="rounded-md border border-border bg-panel p-5">
        ...
        <p className="mt-1 text-sm text-muted-foreground">Restore seeded vehicles, drivers, trips and logs to their original state.</p>
        <Button variant="destructive" size="sm" className="mt-3" onClick={() => { resetDemo(); toast.success("Demo data reset"); }}>
          Reset demo data
        </Button>
      </div>
    </div>
